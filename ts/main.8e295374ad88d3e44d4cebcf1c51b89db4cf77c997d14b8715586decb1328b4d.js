(() => {
  ...
  var ALL_STATES = ["code-closed", "code-minimized", "code-maximized"];
  var DOT_DX = [-1, 19, 39];
  var DOT_DY = 2;
  function setupCodeHeader() {
    ...
    highlights.forEach((highlight) => {
      const setState = (state) => {
        ...
        document.body.classList.toggle(
          "code-fullscreen-open",
          highlight.classList.contains("code-maximized")
        );
      };
      const dots = [
        { cls: "code-dot-red", title: "\u6298\u53E0\u4EE3\u7801\u5757", state: "code-closed" },
        { cls: "code-dot-yellow", title: "\u6700\u5C0F\u5316\u4E3A\u9884\u89C8", state: "code-minimized" },
        { cls: "code-dot-green", title: "\u5168\u5C4F\u663E\u793A", state: "code-maximized" }
      ];
      dots.forEach(({ cls, title, state }, i) => {
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = `code-dot ${cls}`;
        btn.title = title;
        btn.setAttribute("aria-label", title);
        btn.dataset.dotDx = String(DOT_DX[i]);
    ...
    const updatePositions = () => {
      highlights.forEach((highlight) => {
        ...
        highlight.querySelectorAll(".code-dot").forEach((btn) => {
          btn.style.left = `${padLeft + parseFloat(btn.dataset.dotDx)}px`;
          btn.style.top = `${padTop + DOT_DY}px`;
        });
      });
    };
    updatePositions();
    ...
    document.addEventListener("keydown", (e) => {
      ...
      const open = document.querySelector(".highlight.code-maximized");
      if (open) {
        open.classList.remove("code-maximized");
