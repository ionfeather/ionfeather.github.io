(() => {
  ...
  function scrollToTocElement(tocElement, scrollableNavigation) {
    ...
    const margin = Math.max(24, containerRect.height * 0.15);
    const bandTop = containerRect.top + margin;
    const bandBottom = containerRect.bottom - margin;
    let delta = 0;
    if (linkRect.top < bandTop) {
      delta = linkRect.top - bandTop;
    } else if (linkRect.bottom > bandBottom) {
      delta = linkRect.bottom - bandBottom;
    }
    if (delta !== 0) {
      scrollableNavigation.scrollTo({ top: scrollableNavigation.scrollTop + delta, behavior: "auto" });
    }
  }
  ...
  function setupScrollspy() {
    ...
    function scrollHandler() {
      ...
      if (newActiveSection) {
        ...
        while (newActiveSectionLink && newActiveSectionLink.offsetParent === null) {
          newActiveSectionLink = newActiveSectionLink.parentElement?.closest("li");
        }
      }
  ...
  function setupTocFilter() {
    ...
    const toc = widget?.querySelector("#TableOfContents");
    const toggle = widget?.querySelector(".toc-filter-toggle");
    const panel = widget?.querySelector(".toc-filter-panel");
    const depthInput = widget?.querySelector(".toc-filter-depth");
    if (!widget || !toc || !toggle || !panel || !depthInput) return;
    toggle.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
    ...
    const applyFilter = () => {
      const depth = Math.min(5, Math.max(1, parseInt(depthInput.value, 10) || 3));
      toc.querySelectorAll("li").forEach((li) => {
        let level = 0;
        ...
        li.classList.toggle("toc-filtered", level > depth);
        li.classList.toggle("toc-filter-leaf", level === depth);
      });
    };
    depthInput.addEventListener("change", applyFilter);
    applyFilter();
  }
  function setupTocNumbering() {
    const widget = document.querySelector(".toc-widget");
    const toc = widget?.querySelector("#TableOfContents");
    const toggle = widget?.querySelector(".toc-number-toggle");
    if (!toc || !toggle) return;
    toggle.addEventListener("click", (e) => {
      e.preventDefault();
      const on = toc.classList.toggle("toc-numbered");
  ...
  function setupCodeHeader() {
    ...
    highlights.forEach((highlight) => {
      ...
      if (codeEl && lang && lang.toLowerCase() !== "fallback") {
  ...
  // ns-hugo-imp:/Users/lizi/ionfeather/ionfeather-log/assets/ts/headingCollapse.ts
  var HEADING_TAGS = ["H1", "H2", "H3", "H4", "H5", "H6"];
  function setupHeadingCollapse() {
    ...
    article.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach((heading) => {
      ...
      toggle.type = "button";
      toggle.setAttribute("aria-label", "\u6298\u53E0/\u5C55\u5F00\u672C\u8282");
      toggle.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>';
      toggle.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        const collapsed = heading.classList.toggle("heading-collapsed");
        const level = HEADING_TAGS.indexOf(heading.tagName);
        let el = heading.nextElementSibling;
        while (el) {
          if (HEADING_TAGS.includes(el.tagName) && HEADING_TAGS.indexOf(el.tagName) <= level) break;
          el.classList.toggle("hc-hidden", collapsed);
  ...
  // ns-hugo-imp:/Users/lizi/ionfeather/ionfeather-log/assets/ts/readingProgress.ts
  var STORAGE_PREFIX = "reading-progress:";
  var MIN_SAVE_Y = 400;
  var FINISHED_RATIO = 0.95;
  function setupReadingProgress() {
    ...
    if (saved && saved.y > MIN_SAVE_Y) {
      const maxScroll = document.documentElement.scrollHeight - window.innerHeight;
      if (saved.y < maxScroll * FINISHED_RATIO) {
        showResumeToast(Math.min(saved.y, maxScroll));
      }
    }
    const save = () => {
      try {
        if (window.scrollY > MIN_SAVE_Y) {
          localStorage.setItem(key, JSON.stringify({ y: window.scrollY }));
        } else {
    ...
    function showResumeToast(y) {
      let hint = "";
      article.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach((h) => {
        if (h.getBoundingClientRect().top + window.scrollY <= y + 40) {
          hint = (h.textContent || "").trim();
        }
      });
      const toast = document.createElement("div");
      toast.className = "reading-resume";
      toast.innerHTML = `<span class="reading-resume__text">\u4E0A\u6B21\u8BFB\u5230${hint ? `\u300C${hint}\u300D` : "\u8FD9\u91CC"}</span><button type="button" class="reading-resume__go">\u7EE7\u7EED\u9605\u8BFB</button><button type="button" class="reading-resume__close" aria-label="\u5173\u95ED">\xD7</button>`;
      document.body.appendChild(toast);
      const dismiss = () => toast.remove();
      toast.querySelector(".reading-resume__go").addEventListener("click", () => {
        window.scrollTo({ top: y, behavior: "smooth" });
        dismiss();
      });
      toast.querySelector(".reading-resume__close").addEventListener("click", dismiss);
