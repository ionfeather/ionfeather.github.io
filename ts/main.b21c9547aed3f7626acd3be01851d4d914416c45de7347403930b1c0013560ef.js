(() => {
  ...
  function setupTocFilter() {
    ...
    const applyFilter = () => {
      const mode = widget.querySelector('input[name="toc-filter"]:checked')?.value;
      const depth = Math.min(6, Math.max(1, parseInt(depthInput.value, 10) || 2));
      const active = mode === "depth";
      toc.querySelectorAll("li").forEach((li) => {
        let level = 0;
        ...
        li.classList.toggle("toc-filtered", active && level > depth);
        li.classList.toggle("toc-filter-leaf", active && level === depth);
      });
    };
    widget.querySelectorAll('input[name="toc-filter"]').forEach((radio) => {
      radio.addEventListener("change", applyFilter);
    });
    depthInput.addEventListener("change", () => {
      const depthRadio = widget.querySelector('input[name="toc-filter"][value="depth"]');
