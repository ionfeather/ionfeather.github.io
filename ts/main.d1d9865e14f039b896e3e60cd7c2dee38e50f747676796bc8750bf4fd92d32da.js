(() => {
  ...
  var LABEL_DX = CONTAINER_DX + 56.6 + 8;
  var GLYPHS = {
  ...
  function setupCodeHeader() {
    ...
    highlights.forEach((highlight) => {
      ...
      if (codeEl) {
        ...
        label.textContent = codeEl.getAttribute("data-lang");
    ...
    const updatePositions = () => {
      highlights.forEach((highlight) => {
        ...
        const label = highlight.querySelector(".code-lang-label");
        if (label) {
          label.style.left = `${padLeft + LABEL_DX}px`;
          label.style.top = `${padTop + CONTAINER_DY}px`;
        }
      });
    };
    updatePositions();
